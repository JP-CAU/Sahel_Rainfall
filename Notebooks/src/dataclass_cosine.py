...
class Data:
    ...
    def __scale_data_input(self, train, val, test):
        ...
        if self.feature_list is not None:
            scaled_train = scaler.transform(train[:, :-1]) if len(train) > 0 else []
            if len(train) > 0:
                scaled_train = np.concatenate(
                    (scaled_train, train[:, len(self.feature_list) :]), axis=1
                )

            scaled_val = scaler.transform(val[:, :-1]) if len(val) > 0 else []
            if len(val) > 0:
                scaled_val = np.concatenate(
                    (scaled_val, val[:, len(self.feature_list) :]), axis=1
                )

            scaled_test = scaler.transform(test[:, :-1]) if len(test) > 0 else []
            if len(test) > 0:
