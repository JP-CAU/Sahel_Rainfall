...
class Data:
    ...
    def cosine_encode_month(self, months):
        """This function cosine encodes a list of integer months
        ...
        cosine_months = list()
        for month in months:
            angle = 2 * np.pi * (month) / 12  # Convert month to angle in radians
            cosine_encoding = np.cos(angle)  # Calculate cosine value
            cosine_months.append(cosine_encoding)
        return cosine_months
    ...
    def __load_data(self):
        ...
        if self.feature_list is not None:
            climind.drop(
                columns=[
                    col for col in climind.columns if col not in self.feature_list
                ],
                inplace=True,
            )
        ...
        for i in range(len(df_cesm)):
            if i <= 2:
                ...
                new_feat2.append(
                    np.mean(
                        [df_cesm.iloc[i - 1], df_cesm.iloc[i - 2], df_cesm.iloc[i - 3]]
                    )
                )
        for i in range(len(df_foci)):
            if i <= 2:
                ...
                new_feat2.append(
                    np.mean(
                        [df_foci.iloc[i - 1], df_foci.iloc[i - 2], df_foci.iloc[i - 3]]
                    )
                )
        ...
        climind["last_3_month"] = new_feat2
        climind["last_month"] = new_feat3
    ...
    def __scale_data_input(self, train, val, test):
        ...
        if self.feature_list is not None:
            ...
            if len(train) > 0:
                scaled_train = (
                    np.concatenate((scaled_train, train[:, 32:]), axis=1)
                    if self.lead_time > 0
                    else np.concatenate((scaled_train, train[:, 31:]), axis=1)
                )

            scaled_val = scaler.transform(val[:, :-12]) if len(val) > 0 else []
            if len(val) > 0:
                scaled_val = (
                    np.concatenate((scaled_val, val[:, 32:]), axis=1)
                    if self.lead_time > 0
                    else np.concatenate((scaled_val, val[:, 31:]), axis=1)
                )

            scaled_test = scaler.transform(test[:, :-12]) if len(test) > 0 else []
            if len(test) > 0:
                scaled_test = (
                    np.concatenate((scaled_test, test[:, 32:]), axis=1)
                    if self.lead_time > 0
                    else np.concatenate((scaled_test, test[:, 31:]), axis=1)
                )
